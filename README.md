...
控制栏提供观看控制：播放 / 暂停、进度与跳转、播放速度（0.5–1.5 倍），以及音符速度（− / + 每次 0.1，按住 Shift 每次 1，与游戏开演前的音符速度按钮相同；也可直接输入数值）和打开设置面板的“设置”按钮；播放时自动隐藏，移动指针、轻触或按键时显示。设置面板提供游戏自己的 Live 设置（音符速度、判定与音符时机、轨道与界面显示、音量等），按游戏的分组排列，范围与默认值与游戏相同，只列出当前谱面数据支持的项目（见 [docs/api.md](docs/api.md#live-settings)）。控制界面有简体中文、繁体中文、英语、日语、韩语五种语言，跟随页面语言。播放器获得焦点时的键盘操作：Space 或 K 播放 / 暂停，← / → 后退 / 前进 5 秒，↑ / ↓ 调整播放速度，[ / ] 音符速度 −0.1 / +0.1（加 Shift 为 ±1），Esc 关闭设置面板。
