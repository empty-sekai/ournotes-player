...
// GL objects created through the session's context, deleted by dispose(). Only a WebGL2RenderingContext itself is
// tracked (its create / delete methods are wrapped on the instance while the session lives); a wrapped or proxied
// context is used as given and nothing is tracked. The objects are held weakly: one the session has dropped without
// deleting it is left to the garbage collector, as without tracking.
